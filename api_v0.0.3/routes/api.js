...
exports.search = function(req, res){
    var today = new Date();
    try {
        if(req.query.v == null ){ // no parameter handler
            var data = { 'code':'400', 'source': 'no data', 'message': 'Bad Request. No parameter ( v ).'};
            res.header('Contect-type', 'application/json');
            res.header('Charset', 'utf8');
            res.jsonp(data);
        }else if(req.query.v == ''){ // no search value handler
            var data = { 'code':'404', 'source': 'no data', 'message': 'Bad Request. Parameter ( v ) is no value.'};
            res.header('Contect-type', 'application/json');
            res.header('Charset', 'utf8');
            res.jsonp(data);
        }else{
        ...
        var data = { 'code':'500', 'source': 'no data', 'message': ex.message};
        res.header('Contect-type', 'application/json');
        res.header('Charset', 'utf8');
        res.jsonp(data);
    }
